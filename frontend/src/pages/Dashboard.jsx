...
export default function Dashboard({ onUploadComplete }) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8 mt-14 md:mt-0">
      {/* Top Navbar */}
      <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 pl-10 md:pl-0">
        <div className="flex items-center space-x-3 w-full md:w-auto self-start">
        ...
        <div className="hidden md:flex w-10 h-10 bg-[#3730A3] rounded-full items-center justify-center text-white shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        {[
          ...
          <div key={i} className="bg-white rounded-xl p-4 md:p-6 soft-shadow flex items-center space-x-4">
      ...
      <div 
        {...getRootProps()} 
        className={`bg-white rounded-2xl p-6 md:p-12 soft-shadow border-2 border-dashed transition-all cursor-pointer relative overflow-hidden group
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="flex items-center space-x-2 md:space-x-4 mb-6">
            <div className="w-12 h-16 md:w-16 md:h-20 bg-blue-50 rounded-lg flex flex-col items-center justify-center text-blue-600 border border-blue-100 shadow-sm -rotate-6 transform hover:rotate-0 transition-transform">
              <FileText className="w-5 h-5 md:w-6 md:h-6 mb-1" />
              <span className="text-[8px] md:text-[10px] font-bold">PDF</span>
            </div>
            <div className="w-16 h-16 md:w-20 md:h-20 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-600 border-4 border-white shadow-md z-10">
              <Plus className="w-6 h-6 md:w-8 md:h-8" />
            </div>
            <div className="w-12 h-16 md:w-16 md:h-20 bg-slate-50 rounded-lg flex flex-col items-center justify-center text-slate-600 border border-slate-200 shadow-sm rotate-6 transform hover:rotate-0 transition-transform">
              <File className="w-5 h-5 md:w-6 md:h-6 mb-1" />
              <span className="text-[8px] md:text-[10px] font-bold">DOCX</span>
            </div>
          </div>

          <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">Drop your intelligence here</h2>
          <p className="text-sm md:text-base text-gray-500 mb-8 max-w-sm">
            Drag and drop PDF, DOCX, or TXT files to instantly index them for AI-powered querying.
          </p>

          <button className="bg-[#3730A3] hover:bg-[#312e81] text-white px-6 md:px-8 py-3 rounded-lg text-sm md:text-base font-medium shadow-lg shadow-indigo-500/25 transition-all mb-8 flex items-center w-full md:w-auto justify-center">
             <UploadCloud className="w-5 h-5 mr-2" /> 
             {isUploading ? `Uploading... ${uploadProgress}%` : 'Select Local Files'}
          </button>

          <div className="flex items-center space-x-2 text-xs md:text-sm text-gray-400 font-medium">
      ...
      <div className="bg-white rounded-xl soft-shadow overflow-hidden">
        <div className="p-4 md:p-6 flex flex-col md:flex-row md:items-center justify-between border-b border-gray-100 space-y-4 md:space-y-0">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Recently Uploaded</h3>
            <p className="text-xs md:text-sm text-gray-500">Live tracking of your document ingestion pipeline</p>
          </div>
          <div className="flex items-center justify-between md:justify-end space-x-4">
            {selectedDocs.length > 1 && (
              <Link to={`/chat/${selectedDocs.join(',')}`} className="bg-[#3730A3] hover:bg-[#312e81] text-white px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-bold shadow-md transition-all whitespace-nowrap">
                Chat Selected
              </Link>
            )}
            <button className="text-xs md:text-sm font-medium text-indigo-600 hover:text-indigo-700 flex items-center whitespace-nowrap">
                ...
                <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-6 flex items-center">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-gray-900 truncate max-w-[150px] md:max-w-[250px]">{doc.filename}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-[#3730A3] text-white rounded-2xl p-6 md:p-8 relative overflow-hidden group hover:shadow-xl transition-all">
          ...
          <h3 className="text-xl md:text-2xl font-bold mb-4 relative z-10">Connect your entire<br/>workspace.</h3>
          <p className="text-indigo-200 text-xs md:text-sm mb-8 max-w-sm relative z-10">
            Sync with Google Drive, Notion, or Slack to automatically feed documents into your AI library.
          </p>
          <button className="bg-white text-indigo-900 px-4 md:px-6 py-2 md:py-2.5 rounded-lg text-xs md:text-sm font-bold flex items-center hover:bg-indigo-50 transition-colors relative z-10">
            Explore Integrations <ArrowRight className="w-4 h-4 ml-2" />
          </button>
        </div>
        
        <div className="bg-[#DBEAFE] rounded-2xl p-6 md:p-8 text-center flex flex-col items-center justify-center relative overflow-hidden">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-100/50 rounded-2xl flex items-center justify-center text-[#3730A3] mb-4">
            <ShieldCheck className="w-6 h-6 md:w-8 md:h-8" />
          </div>
          <h3 className="text-base md:text-lg font-bold text-gray-900 mb-2">Secure & Private</h3>
          <p className="text-xs md:text-sm text-gray-600 mb-6 max-w-xs">
            All documents are encrypted and siloed. Your data is never used to train global models.
          </p>
          <div className="flex flex-wrap justify-center gap-2">
            <span className="px-3 py-1 bg-white/50 border border-white text-gray-600 text-[10px] md:text-xs font-bold rounded-full">SOC2 Compliant</span>
            <span className="px-3 py-1 bg-white/50 border border-white text-gray-600 text-[10px] md:text-xs font-bold rounded-full">AES-256</span>
          </div>
        </div>
      </div>
    </div>
